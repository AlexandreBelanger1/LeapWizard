...
@onready var world_gen_timer = $WorldGenTimer
var worldX = 0
var worldY = 0
...
func GenerateLevel(ySize,xSize):
	...
	world_gen_timer.start()

@onready var wait_2_seconds = $"Wait 2 seconds"

func generateStep(index: int):
	if index == 0:
		GenerateRunes(worldY,worldX)
		GeneratePathways(worldY,worldX)
		GenerateLargeFeatures(worldY,worldX)
		GenerateFeatures(worldY,worldX)
		GenerateGround(worldX)
		FillTiles(worldY,worldX)
		...
		world_gen_timer.stop()
		camera_2d.setTarget(Vector2(0,0))
		wait_2_seconds.start()
...
func _on_world_gen_timer_timeout():
	generateStep(generationStep)
	generationStep += 1


func _on_wait_2_seconds_timeout():
	camera_2d.setTarget(Vector2(-999,-999))
	camera_2d.zoom = Vector2(3,3)
	player.pausePlayer(false)

func viewWorldGen():
	player.pausePlayer(true)
	camera_2d.setTarget(Vector2(2212,-512))
